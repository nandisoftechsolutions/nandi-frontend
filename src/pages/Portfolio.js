...
function Portfolio() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/manageprojects");
  ...
  return (
    <div className="container py-5">
      ...
      <div className="row g-4">
        {filtered.map(({ id, title, description, full_image_url, youtube_link, demo_link }) => (
          <div key={id} className="col-12 col-sm-6 col-lg-3">
            ...
            >
              {full_image_url ? (
                <img src={full_image_url} className="card-img-top" alt={title} style={{ objectFit: "cover", height: "180px" }} />
              ) : (
                <div style={{ height: "180px", background: "#eee", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  No Image Available
                </div>
              )}
              <div style={{ ...overlayStyle, opacity: hoveredId === id ? 1 : 0 }}></div>
              <div className="card-body">
                <h5 className="card-title text-primary">{title}</h5>
                <p className="text-muted small">{description}</p>
                <div className="d-flex justify-content-between">
                  {youtube_link && <a href={youtube_link} target="_blank" rel="noreferrer" className="btn btn-sm btn-danger">YouTube</a>}
                  {demo_link && <a href={demo_link} target="_blank" rel="noreferrer" className="btn btn-sm btn-success">Live Demo</a>}
                </div>
              </div>
            </div>
          </div>
